	...
	public class CharacterThirdPerson : CharacterBase {
		...
		private Vector3 GetLookDirection() {
			...
			case MoveMode.Directional:
				...
				return lookInCameraDirection? inputState.lookPos - GetComponent<Rigidbody>().position: transform.forward;
			case MoveMode.Strafe:
				if (isMoving) return inputState.lookPos - GetComponent<Rigidbody>().position;
				return lookInCameraDirection? inputState.lookPos - GetComponent<Rigidbody>().position: transform.forward;
			}
